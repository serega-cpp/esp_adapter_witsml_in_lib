	...
	for (pugi::xml_node_iterator node = nodes->children().begin(); node != nodes->children().end(); ++node) {
            ...
            if (Utils::strcmpi(node_name, witsml_rule.variable_node_name.c_str()) == 0) {
                ...
                for (size_t i = 0; i < vv.size(); i++) {
                    if (witsml_rule.variable_columns[i].type == WitsmlRule::Item::TzCalcType)
                        vv[i].assign(process_timezone_field(vv, witsml_rule.variable_columns[i].indexes));
                }
                for (size_t i = 0; i < vv.size(); i++) {
                    ...
                    if (witsml_rule.variable_columns[i].type == WitsmlRule::Item::HashCalcType)
                        ...
                        vv[i].assign(process_time_field(vv[i]));
                }
            }
            // process data values
        ...
        for (size_t i = 0; i < static_values.size(); i++) {
            if (witsml_rule.static_columns[i].type == WitsmlRule::Item::TzCalcType)
                static_values[i].assign(process_timezone_field(static_values, witsml_rule.static_columns[i].indexes));
        }
        for (size_t i = 0; i < static_values.size(); i++) {
            ...
            if (witsml_rule.static_columns[i].type == WitsmlRule::Item::HashCalcType)
                ...
                static_values[i].assign(process_time_field(static_values[i]));
        }
